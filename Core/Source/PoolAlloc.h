...
public:
	...
	// mDbgBoundary variable set to 0xdeadbeef.
	PoolAllocator();
...
{
	DbgAssert( size <= blockSize, "Stack out of memory" );
	
	if ( mBlocksFree > 0 )
	{
		void* usableBlock = ( void* )mFreeList->mMemory;
		mFreeList = mFreeList->mNext;
		mBlocksFree--;
		return usableBlock;
	}
	DbgAssert( mBlocksFree > 1, "No blocks available" );
	return nullptr;
}
...
#ifdef POOL_ALLOC_DEBUG
	DbgAssert( block->mDbgBoundary == 0xdeadbeef, "Bounds were overwritten!" );
