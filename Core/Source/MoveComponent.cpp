...
void MoveComponent::Tick(float deltaTime)
{
	...
	// Velocity Vertlet integration, apply force to object
	Vector3 pos = mOwner.GetPosition();
	Vector3 avgVelocity = mVelocity + ( mAcceleration * deltaTime ) / 2.0f;
	pos += avgVelocity * deltaTime + inputVelocity * deltaTime;
	mAcceleration = mForceInternal / mMass;
	mVelocity = avgVelocity + ( mAcceleration * deltaTime ) / 2.0f;
...
void MoveComponent::SetProperties(const rapidjson::Value& properties)
{
	...
	// Linear speed, horizontal speed, yaw speed, and pitch speed values set from JSON
	float linearSpeed;
	if ( GetFloatFromJSON( properties, "linearSpeed", linearSpeed ) )
	{
		SetLinearSpeed( linearSpeed );
	}

	float horizSpeed;
	if ( GetFloatFromJSON( properties, "horizontalSpeed", horizSpeed ) )
	{
		SetHorizontalSpeed( horizSpeed );
	}

	float yawSpeed;
	if ( GetFloatFromJSON( properties, "yawSpeed", yawSpeed ) )
	{
		SetYawSpeed( yawSpeed );
	}

	float pitchSpeed;
	if ( GetFloatFromJSON( properties, "pitchSpeed", pitchSpeed ) )
	{
