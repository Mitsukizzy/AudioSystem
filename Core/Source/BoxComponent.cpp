...
void BoxComponent::OnUpdatedTransform()
{
	...
	mWorldSpaceBounds.mMin = mModelSpaceBounds.mMin * mScale * mOwner.GetScale() + mOwner.GetPosition();
	mWorldSpaceBounds.mMax = mModelSpaceBounds.mMax * mScale * mOwner.GetScale() + mOwner.GetPosition();
}
...
void BoxComponent::SetProperties(const rapidjson::Value& properties)
{
	...
	// Mesh and Scale properties set from JSON
	std::string mesh;
